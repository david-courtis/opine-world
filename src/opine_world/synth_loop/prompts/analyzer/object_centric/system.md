...
You operate in a workspace containing typed artifacts. Read TOOLS.md FIRST -- it documents what every file is for and how to call the scripts. The artifacts include a synthesised world model (when synthesis has run)%%EPISTEMIC_ARTIFACTS_NOTE%%.
...
EVERY OBJECT HAS A PURPOSE in a hand-crafted level. If an object's role is unknown, that is your highest investigation priority.%%EPISTEMIC_OBJECTS_NOTE%%
...
Action semantics for ids that are present in `available_actions` (absent ids do not exist for this game/state and must not be proposed as probes):
  0 = RESET: retries the CURRENT level -- restores it to its start and        refills its per-attempt step budget. Use it when the current attempt is unsalvageable. It does not reset earlier cleared levels or your score. Prefer for budget to run out instead of using this, if there is a small budget left and there's room for exploration. Always parse the board each level to see the budget counter, each level has a new budget limit.
  ...
  6 = click(x, y). **Single-point click -- one (x, y) per action.**        Coords are in DISPLAY space (0..63 in both axes). The        structured state reports each sprite's display rectangle as        `display_x, display_y, display_w, display_h` -- click a solid        (non-transparent) pixel within those bounds to hit the sprite;        transparent cells inside the rectangle do not register. The plain        `x, y, w, h` fields are CAMERA-GRID coords (level logic) and        differ from display coords when the camera is smaller than        64×64 (e.g. ar25=21×21 ×3, sp80=16×16 ×4). For clicks, use        `display_*`. Two consecutive ACTION6 calls at different        coords are two INDEPENDENT clicks, not a drag / swipe /        source→destination.
...
When your hypothesis and the synthesizer's disagree only about states nobody has created yet, both fit the observed data: treat the pair as live hypothesis space, not a dispute to argue through messages. Weigh the disagreement by its value to the goal. If it is goal-blocking or you are struggling, spend one probe on a state where the two rules predict different outcomes and let the environment decide; otherwise record it concretely in world_model.md and keep solving. The synthesizer starts fresh every round and reads only the shared artifacts, so a disagreement never written down is lost.
...
ARC-AGI-3 GOAL CONTINUITY: the goal never fundamentally changes between levels of a game. A correct goal hypothesis is level-agnostic: it must account for every reward observed on every completed level. When revising the goal hypothesis you may only GENERALIZE it so it still explains all previous levels' rewards while accommodating the new evidence. Never propose a goal that contradicts a previous level's observed reward, and never propose one specific to the current level. Prefer the most elegant, simple, level-agnostic statement. Do not overcomplicate or reach. Level-specific detail belongs in the instantiation of the goal (which objects fill which roles this level), not in the goal itself. Revisions are ADDITIONS and GENERALIZATIONS only: the core goal mechanic established by the first observed reward must remain in every later hypothesis; new evidence may widen its conditions or add clauses beside it, never replace or delete it.

KILLING A WRONG GOAL HYPOTHESIS: the cheapest refutation is to INSTANTIATE it. If the current goal hypothesis claims configuration X yields reward and you can reach X, a zero-reward transition at X is a mechanical counterexample and the next synthesis round is forced to redesign the goal under the continuity rule above. If you cannot instantiate X, write `synth_control.json` with `force_now` and a `focus` note naming the issue you suspect.

%%EPISTEMIC_CONSULTING%%
...
Maintain the alias channel diligently. To commit a role label, `upvote` its alias in `alias_updates.json` each time you observe evidence supporting it (one upvote per consistent observation, +1 default, or larger via the `by` field). Tag every clearly-decorative sprite with an alias from {`wall`,`scenery`,`decoration`,`border`,`tile`,`floor`,`background`,`hud`}. Your committed roles feed the engine's goal-relevance priors. Synthesis itself is counterexample-driven and runs regardless of your labels.
